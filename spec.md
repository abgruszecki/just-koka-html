...
- Tree-doc allowlist is now `931/1590` (`ruby.dat` is `17/21`, `tests19.dat` is `34/103`, `domjs-unsafe.dat` is `26/49`, `webkit01.dat` is `34/54`, and `webkit02.dat` is `24/45`) driven mostly by improved table foster parenting (including `<input>` + select-in-table), better `AfterHead` `<title>` handling, more accurate EOF/template parse error counting, in-body “button scope” fixes (`<button>`-in-scope and `p`-in-button-scope), tokenizer EOF edge-case fixes for `</` in script/RCDATA/RAWTEXT end-tag-open states + case-insensitive appropriate-end-tag matching in rawtext/script/RCDATA (plus `eof-in-tag-name`/`eof-in-tag` and an extra whitespace-in-end-tag-open error to match webkit fixtures), a minimal foreign-content tokenizer guard (avoid switching rawtext/RCDATA on SVG `<title>`), improved error counting for EOF-in-attribute-value (webkit02), improved foreign end-tag mismatch error counting, improved foreign attribute adjustment (whitelist-based) for `xml:`/`xlink:`, and refined `AfterAfterBody` handling to ignore stray `</body>`/`</html>` without extra errors (webkit01).
