...
- Tree construction: minimal insertion-mode slice for wrapper synthesis (auto `<html>/<head>/<body>`), plus basic in-body `p`-closing on block starts (now includes `<dd>/<dt>`), tokenizer auto-switching for rawtext/RCDATA/script/plaintext elements (including `<noscript>` as RAWTEXT when scripting is enabled) with a minimal foreign-content guard (avoid switching on SVG `<title>`), namespace-aware element creation + attribute adjustment for foreign content (keeps `xml:lang` / `xlink:href` *as-is* on HTML elements, but maps them for SVG/Math; foreign-attribute adjustment for `xml:` is now whitelist-based, matching html5lib’s expectations), template nodes (with `content`), a first table-aware slice (table/table-body/row/cell + colgroup handling, plus foster parenting that avoids template/foreign-content regressions and implied `<colgroup>` insertion for `<col>` in `InTable`; plus foster-parenting `<input>` out of tables, and `InColumnGroup` now reprocesses non-whitespace characters in `InTable` and prevents stray `</html>`/`</body>` from popping the real elements, and all table modes now ignore `</html>`/`</body>` end tags to avoid closing the document mid-table; plus a small select-in-table slice that foster-parents `<select>` out of tables when nested table tokens appear, matching `tables01.dat`, and a minimal “in-select” behavior for `<hr>/<option>/<optgroup>` (pop `<option>/<optgroup>` first so they become siblings)); plus more accurate table-mode end-tag error counting (including foreign-content exceptions and treating `style`/`script`/`title` as allowed table children for fixture compatibility), frameset support with explicit `AfterAfterFrameset` mode (ensures trailing comments are siblings of `<html>`, whitespace attaches under `<html>`, and non-HTML tokens like `<plaintext>` are ignored) and per-character error counting for plain text after `</html>` in frameset documents (matches `expected-eof-but-got-char` counts), plus allowing `<html>` attribute merges after `</html>` in frameset documents; and wiring for html5lib `#script-on/off` to the `scripting-enabled` parse option (with more complete `InHeadNoscript` behavior including `<html ...>` attribute merging, `<style>/<noframes>` rawtext handling, and special `</br>` + EOF cases); `AfterBody` no longer pops `<body>` on `</body>` (matching spec) and closes `<body>` on comment insertion so trailing comments become siblings as expected; `BeforeHead`/`AfterHead` now ignore unexpected end tags instead of implicitly inserting `<head>/<body>` (enables more `tests19.dat` cases where comments must land between `<head>` and `<body>`); `InBody` heading end-tags now pop to the nearest open heading (not necessarily the matching heading name), matching html5lib’s `end-tag-too-early` behavior; `AfterHead` now (1) treats a stray `<head>` start tag as a parse error (matching html5lib’s `unexpected-start-tag` count) and (2) inserts empty `<style>/<script>` tags into the existing `<head>` node rather than moving them into `<body>` (covers a chunk of `tests3.dat`); EOF error counting now includes missing non-optional end tags in `<head>` (e.g. `<script>` at EOF) and `eof-in-attribute-value-double-quote`; tree dump normalizes CR→LF to match the harness; comment/bogus-comment error counting now matches html5lib’s dual legacy+modern buckets (e.g. `<?...` and `<!...`), and `<title>` text stays in-head instead of being reprocessed into the body; fragment parsing now selects the tokenizer’s initial state for *HTML* fragment contexts (e.g. `<title>` → RCDATA) without breaking SVG integration points; plus a pragmatic adoption-agency slice for misnested inline formatting (splitting inline formatting elements like `<b>` and wrapping already-inserted content under special elements), coalescing adjacent text nodes even across ignored tags, and preserving leading whitespace in `<head>` when a character token is mixed whitespace+non-whitespace; allowlists currently `988/1590` (doc) and `51/192` (frag).
...
- Tree-doc allowlist is now `988/1590` (`doctype01.dat` is `30/37`, `ruby.dat` is `17/21`, `tests19.dat` is `52/103`, `tests2.dat` is `41/63`, `tests7.dat` is `22/33`, `tests10.dat` is `23/54`, `tests20.dat` is `47/64`, `domjs-unsafe.dat` is `26/49`, `webkit01.dat` is `37/54`, `webkit02.dat` is `25/45`, and `template.dat` is `62/111`) driven mostly by improved DOCTYPE error counting/ordering (enables more of `doctype01.dat`), improved table foster parenting (including `<input>` + select-in-table), better `AfterHead` `<title>` handling, more accurate EOF/template parse error counting, in-body “button scope” fixes (`<button>`-in-scope and `p`-in-button-scope), tokenizer EOF edge-case fixes for `</` in script/RCDATA/RAWTEXT end-tag-open states + case-insensitive appropriate-end-tag matching in rawtext/script/RCDATA (plus `eof-in-tag-name`/`eof-in-tag` and an extra whitespace-in-end-tag-open error to match webkit fixtures), a minimal foreign-content tokenizer guard (avoid switching rawtext/RCDATA on SVG `<title>`), improved error counting for EOF-in-attribute-value (webkit02), improved foreign end-tag mismatch error counting, improved foreign attribute adjustment (whitelist-based) for `xml:`/`xlink:`, refined `AfterAfterBody` handling to ignore stray `</body>`/`</html>` without extra errors (webkit01), spec-like error counting for `<rp>/<rt>` when nested under non-ruby elements (webkit01), and more correct `</body>` handling (don’t pop; fix comment placement after `</body>`), plus `AfterHead` unexpected-end-tag handling (allows comments between `<head>` and `<body>`), heading end-tag behavior (`end-tag-too-early`), better `<select>` option auto-closing, correct `<dd>/<dt>` auto-closing, and stronger frameset handling via `AfterAfterFrameset` (including `expected-eof-but-got-char` per-character counts) plus merging `<html>` attrs after `</html>` in frameset documents.
- Not implemented yet (high-level): full HTML5 treebuilder insertion modes + table modes + adoption agency edge cases (especially involving table foster parenting), foreign content/templates beyond the current slice, error locations/codes, encoding + byte input.
