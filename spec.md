...
- Tree-doc allowlist is now `880/1590` (+117; `tables01.dat` is `19/19`, `tests20.dat` is `46/64`, `template.dat` is `53/111`, and `webkit01.dat` is `22/54`) driven mostly by improved table foster parenting (including select-in-table), better `AfterHead` `<title>` handling, more accurate EOF/template parse error counting, in-body “button scope” fixes (`<button>`-in-scope and `p`-in-button-scope), tokenizer EOF edge-case fixes for `</` in script/RCDATA/RAWTEXT end-tag-open states + case-insensitive appropriate-end-tag matching in rawtext/script/RCDATA (plus `eof-in-tag-name`/`eof-in-tag` and an extra whitespace-in-end-tag-open error to match webkit fixtures), improved error counting for `< /A>` (adds `expected-tag-name` alongside `invalid-first-character-of-tag-name`), improved error counting for `<tag/` cases (counts both `unexpected-character-after-solidus-in-tag` and `unexpected-solidus-in-tag` for start tags), better handling of misnested formatting end tags around tables (including matching html5lib’s double-error counting in `InTable` for `</font>`-style “table voodoo” cases), MathML attribute name adjustment (`definitionurl` → `definitionURL`), and correct `p`-closing before `<plaintext>` insertion.
