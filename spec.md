...
- Tree-doc allowlist is now `813/1590` (+50; `tables01.dat` is `19/19`, and `template.dat` is `53/111`) driven mostly by improved table foster parenting (including select-in-table), better `AfterHead` `<title>` handling, and more accurate EOF/template parse error counting.
