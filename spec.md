...
- Tokenizer: passes the currently allowlisted tokenizer subset (6595/6810 = 96.8%) (includes non-`Data` `initialStates` + `lastStartTag`, spec-like rawtext/RCDATA/script end-tag recognition, a proper comment state machine (fixes `<!-->`/`--!>` edge cases), and a corrected named-entities table generator that preserves non-BMP codepoints while escaping “unsafe” bidi controls; plus harness support for html5lib tokenizer fixtures with `doubleEscaped: true`, and a tokenizer change to preserve NULs inside `CDATA section state` to match html5lib expectations without regressing NUL→U+FFFD elsewhere).
- Tree construction: minimal insertion-mode slice for wrapper synthesis (auto `<html>/<head>/<body>`), plus basic in-body `p`-closing on block starts, tokenizer auto-switching for rawtext/RCDATA/script/plaintext elements, namespace-aware element creation + basic attribute prefix mapping (`xlink:`/`xml:`/`xmlns:`), template nodes (with `content`), and a first table-aware slice (table/table-body/row/cell + colgroup handling, plus foster parenting that avoids template/foreign-content regressions); allowlists currently `407/1590` (doc) and `24/192` (frag).
