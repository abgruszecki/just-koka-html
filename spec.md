...
- Tree-doc allowlist is now `927/1590` (`ruby.dat` is `17/21`, `tests19.dat` is `34/103`, `domjs-unsafe.dat` is `26/49`, `webkit01.dat` is `33/54`, and `webkit02.dat` is `21/45`) driven mostly by improved table foster parenting (including `<input>` + select-in-table), better `AfterHead` `<title>` handling, more accurate EOF/template parse error counting, in-body “button scope” fixes (`<button>`-in-scope and `p`-in-button-scope), tokenizer EOF edge-case fixes for `</` in script/RCDATA/RAWTEXT end-tag-open states + case-insensitive appropriate-end-tag matching in rawtext/script/RCDATA (plus `eof-in-tag-name`/`eof-in-tag` and an extra whitespace-in-end-tag-open error to match webkit fixtures), a minimal foreign-content tokenizer guard (avoid switching rawtext/RCDATA on SVG `<title>`), improved error counting for EOF-in-attribute-value (webkit02), improved foreign end-tag mismatch error counting, improved foreign attribute adjustment (whitelist-based) for `xml:`/`xlink:`, improved error counting for `< /A>` (adds `expected-tag-name` alongside `invalid-first-character-of-tag-name`), improved error counting for `<tag/` cases (counts both `unexpected-character-after-solidus-in-tag` and `unexpected-solidus-in-tag` for start tags), in-body duplicate `<body>` handling (merge missing attributes into the existing `<body>` while keeping first values), ruby annotation handling for `<rb>/<rt>/<rp>/<rtc>` (generate implied end tags so `<rp>/<rt>` don’t get nested under `<p>`), a small AfterBody slice (`</br>` after `</body>` inserts `<br>` into the existing `<body>` and counts the extra “token after body” parse error; whitespace after `</body>` is inserted into the existing body node), an `AfterAfterBody` slice so `</html><!--comment-->` attaches the comment to the document (matches `webkit01.dat` cases), a “closed html” fallback in `append-into-body` so tokens after `</html>` can still be inserted into the existing `<body>` node when the open-element stack no longer contains `<html>`, and `</html>` handling in this “closed html” scenario to switch back to `AfterAfterBody` so trailing comments don’t get appended into the closed body.
