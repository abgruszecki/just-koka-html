...
- Tree-doc allowlist is now `849/1590` (+86; `tables01.dat` is `19/19`, `tests20.dat` is `46/64`, and `template.dat` is `53/111`) driven mostly by improved table foster parenting (including select-in-table), better `AfterHead` `<title>` handling, more accurate EOF/template parse error counting, in-body “button scope” fixes (`<button>`-in-scope and `p`-in-button-scope), and a tokenizer EOF edge-case fix for `</` in script/RCDATA/RAWTEXT end-tag-open states.
