...
- Tree-doc allowlist is now `845/1590` (+82; `tables01.dat` is `19/19`, `tests20.dat` is `46/64`, and `template.dat` is `53/111`) driven mostly by improved table foster parenting (including select-in-table), better `AfterHead` `<title>` handling, more accurate EOF/template parse error counting, and in-body “button scope” fixes (`<button>`-in-scope and `p`-in-button-scope).
