...
- Tokenizer: passes the currently allowlisted tokenizer subset (6610/6810 = 97.1%) (includes non-`Data` `initialStates` + `lastStartTag`, spec-like rawtext/RCDATA/script end-tag recognition, more correct end-tag-open handling for `</>` (no spurious character tokens), duplicate-attribute dropping (keeps first value), and a fuller comment state machine (including `<!`/`<!--` sequences inside comments); plus harness support for html5lib tokenizer fixtures with `doubleEscaped: true`, and tokenizer fixes for CDATA NUL handling and script “HTML comment”/double-escape behavior (bringing `domjs.test` to 43/43 passing)).
